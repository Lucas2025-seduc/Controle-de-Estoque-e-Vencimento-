...
const PRECACHE_URLS = [
  ...
  './icon-192.png',
  './icon-512.png',
  // CDNs Externos (Crucial para funcionar offline)
...
self.addEventListener('fetch', (event) => {
  ...
  // Estratégia para CDNs e Arquivos Estáticos: Stale-While-Revalidate
  // Carrega rápido do cache, mas atualiza no fundo
  if (PRECACHE_URLS.includes(url.pathname) || url.origin !== location.origin) {
    event.respondWith(
      caches.match(event.request).then((cachedResponse) => {
        const fetchPromise = fetch(event.request).then((networkResponse) => {
          // Atualiza o cache com a nova versão se disponível
          ...
          return networkResponse;
            ...
            // Falha silenciosa na atualização de fundo se offline
        });
      ...
      fetch(event.request)
        ...
        .catch(() => {
          return caches.match(event.request).then((response) => {
             // Se falhar tudo e for navegação, retorna o index (Single Page App)
